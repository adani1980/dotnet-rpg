...
namespace Dotnet_rpg.Services.Fights
{
    public class FightService : IFightService
    {
        ...
        public FightService(DataContext contex, IMapper mapper)
        {
            _mapper = mapper;
            _contex = contex;
        }
                ...
                while (!defeated)
                {
                    foreach (var attacker in characters)
                    {
                        ...
                        if (opponent.HitPoints <= 0)
                        {
                            ...
                            response.Data.Log.Add($"{attacker.Name} wins with {attacker.HitPoints} HitPoints left!");
                            break;
                        }
                    }
                }
        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack(SkillAttackDto request)
        {
            ...
            try
            {
                var attacker = await _contex.Characters
                    .Include(c => c.Skills)
                    .FirstOrDefaultAsync(c => c.Id == request.AttackerId);

                var opponent = await _contex.Characters
                    .FirstOrDefaultAsync(c => c.Id == request.OpponentId);

                var skill = await _contex.Skills
